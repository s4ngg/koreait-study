...
public class BoardFileServiceImpl implements BoardFileService {
	private final BoardFileRepository boardFileRepository; // 게시판 DB 접근

	// 파일 업로드 경로
	private static final String UPLOAD_DIR = System.getProperty("user.dir")
			+ "/src/main/resources/static/uploads/board";
	...
	private SaveFile saveFileToDisk(MultipartFile file, File dir) {
		...
		// 원본 파일명이 없거나 공백이면 기본 파일명(unknown)으로 대체한다.
		...
		// 파일명에서 마지막 .의 위치(인덱스) 찾기. (~~~.exe 이런식이니깐)
		// 없을 경우 -1을 반환한다.
		int dotIndex = originalFileName.lastIndexOf('.');

		// 파일명에서 .이 존재한다면
		if (dotIndex > -1) {
			// 찾은 .의 인덱스부터 문자열을 잘라 확장자를 가져온다.
